...
object Devices extends Controller {
  ...
  def find(deviceUuid: String) = Action.async { implicit req =>
    DeviceRepository.getByDeviceId(deviceUuid).map { eltOpt =>
  ...
  def details(uuid: DeviceId) = Action.async { implicit req =>
    repository.getByUuid(uuid).map {
      _.map { elt => Ok(Writer.write(elt)) }.getOrElse(NotFound)
    }
  }

  def actions(uuid: DeviceId) = Action.async { implicit req =>
  ...
  def eventActions(uuid: DeviceId, eventId: EventId) = Action.async { implicit req =>
