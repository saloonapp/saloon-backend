...
object Event {
  ...
  def fromMap(d: Map[String, String]): Option[Event] =
    if (d.get("name").isDefined) {
      Some(Event(
        ...
        d.get("published").map(_.toBoolean),
  ...
  def toMap(e: Event): Map[String, String] = Map(
    ...
    "published" -> e.published.getOrElse(false).toString,
...
object EventUI {
  ...
  def toModel(d: EventUI): Event = Event(d.uuid, d.image, d.name, d.description, None, d.start, d.end, d.address, d.twitterHashtag, Some(d.published), d.created, d.updated)
  def fromModel(m: Event, sessionCount: Int, exponentCount: Int): EventUI = EventUI(m.uuid, m.image, m.name, m.description, m.start, m.end, m.address, m.twitterHashtag, m.published.getOrElse(false), m.created, m.updated, sessionCount, exponentCount)
}
...
object EventData {
  ...
  def toModel(d: EventData): Event = Event(Repository.generateUuid(), Some(d.image), d.name, d.description, None, d.start, d.end, d.address, d.twitterHashtag.map(toHashtag), Some(d.published), new DateTime(), new DateTime())
  def fromModel(m: Event): EventData = EventData(m.name, m.description, m.image.getOrElse(""), m.start, m.end, m.address, m.twitterHashtag, m.published.getOrElse(false))
  def merge(m: Event, d: EventData): Event = m.copy(name = d.name, description = d.description, image = Some(d.image), start = d.start, end = d.end, address = d.address, twitterHashtag = d.twitterHashtag.map(toHashtag), published = Some(d.published), updated = new DateTime())
