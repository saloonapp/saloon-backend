...
object RivieraDev extends Controller {
  ...
  case class RivieraDevAttendee(name: String, description: String, company: Option[String], avatar: String, profilUrl: Option[String], social: AttendeeSocial) {
    def toAttendee(eventId: String): Attendee = Attendee(Repository.generateUuid(), eventId, this.name, this.description, AttendeeImages(this.avatar), AttendeeInfo("speaker", "", this.company.getOrElse(""), this.profilUrl), social, AttendeeMeta(Some(DataSource(this.name, "RivieraDev API", "http://www.rivieradev.fr/apiv1/talks")), new DateTime(), new DateTime()))
  }
