...
object FileImporter {
  ...
  def importEvents(importedFile: Reader, cfg: FileImportConfig): Future[(Int, List[Map[String, String]])] = {
    importData(Event.fromMap, EventRepository.drop, EventRepository.bulkInsert)(importedFile, cfg)
  }

  def importExponents(importedFile: Reader, cfg: FileImportConfig, eventId: String): Future[(Int, List[Map[String, String]])] = {
  ...
  def importSessions(importedFile: Reader, cfg: FileImportConfig, eventId: String): Future[(Int, List[Map[String, String]])] = {
  ...
  private def importData[T](formatData: Map[String, String] => Option[T], deleteOld: () => Future[Boolean], bulkInsert: List[T] => Future[Int])(importedFile: Reader, cfg: FileImportConfig): Future[(Int, List[Map[String, String]])] = {
