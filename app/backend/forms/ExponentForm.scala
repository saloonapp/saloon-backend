...
object ExponentCreateData {
  ...
  def toModel(d: ExponentCreateData): Exponent = Exponent(ExponentId.generate(), d.eventId, None, d.name, Jsoup.parse(d.descriptionHTML).text(), d.descriptionHTML, toImages(d), toInfo(d, List()), toConfig(d), toMeta(d))
