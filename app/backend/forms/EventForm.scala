...
object EventCreateData {
  ...
  def toModel(d: EventCreateData): Event = Event(EventId.generate(), d.ownerId, d.name, Jsoup.parse(d.descriptionHTML).text(), d.descriptionHTML, toImages(d), toInfo(d), EventEmail(None), toConfig(d), toMeta(d))
