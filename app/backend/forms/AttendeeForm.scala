...
object AttendeeCreateData {
  ...
  def toModel(d: AttendeeCreateData): Attendee = Attendee(AttendeeId.generate(), d.eventId, d.info.firstName + " " + d.info.lastName, Jsoup.parse(d.descriptionHTML).text(), d.descriptionHTML, toImages(d), toInfo(d.info), None, d.social, List(), toMeta(d))
