...
object SessionCreateData {
  ...
  def toModel(d: SessionCreateData): Session = Session(Repository.generateUuid(), d.eventId, d.name, Jsoup.parse(d.descriptionHTML).text(), d.descriptionHTML, toImages(d), toInfo(d), toMeta(d))
